...
*   **i3 Window Manager:** Configured with clean 2px pixel borders (titlebars removed), active window highlighted with a gold/amber border (`#ebcb8b`), shortcuts for Alacritty, Vicinae, Rofi (`Alt+Tab`), minimize/restore (`Alt+m` / `Alt+Shift+m`), and lock screen (`Alt+Shift+x`).
